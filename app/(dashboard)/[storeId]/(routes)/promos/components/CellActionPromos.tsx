...
import { PromoColumn } from "./ColumnsPromos";
...
const CellAction: FC<CellActionProps> = ({ data }) => {
  ...
  const onCopy = () => {
    navigator.clipboard.writeText(data.userAccessCode);
    toast.success("Promocode copied to clipbiard");
  };
    ...
    <AlertModal loading={loading} onConfirm={deleteHandler} isOpen={open} onClose={()=>setOpen(false) }/>
      <DropdownMenu>
        ...
        <DropdownMenuContent>
          ...
          <DropdownMenuItem
            onClick={() =>
              router.push(`/${params.storeId}/promos/${data.id}`)
            }
          >
